...
`openadapt-flow seal ./bundle-v2 --out ./bundle-prod`. The command preserves the
source, refuses an existing destination, encrypts `workflow.json` and template
crops with AES-256-GCM, and verifies the resulting integrity digest. Durable
checkpoints use the same environment key; keyed loads decrypt in memory and
fail on a missing/wrong key or tampering.
