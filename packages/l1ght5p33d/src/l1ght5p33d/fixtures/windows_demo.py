...
def build_fixture(output: Path) -> Path:
    ...
    source = Path(str(files("l1ght5p33d.fixtures").joinpath("CreatorFixture.cs")))
    output.mkdir(parents=True, exist_ok=True)
    executable = output / "L1ght5p33dFixture.exe"
...
def run_demo() -> int:
    if sys.platform != "win32":
        ...
        return 2
    output = Path(tempfile.mkdtemp(prefix="l1ght5p33d-windows-"))
    ...
    try:
        print(
            "Click the L1ght5p33d Creative Fixture window once to begin (60 seconds).",
            flush=True,
        )
